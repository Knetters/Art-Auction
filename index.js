...
app.get('/game/:gameCode', (req, res) => {
  const iconValue = "ingameIcons";
  const gameCode = req.params.gameCode;
  const drawTime = req.params.drawTime;

  if (lobbies[gameCode]) {
    const players = lobbies[gameCode].players;
    const playerRole = req.cookies.playerRole;

    res.render('game', { iconValue, gameCode, players, playerRole, drawTime });
  } else {
...
app.get('/lobby/:lobbyCode', (req, res) => {
  ...
  if (lobbies[lobbyCode]) {
    ...
    res.render('lobby', { iconValue, gameCode: lobbyCode, players, playerRole, drawTime });
  } else {
...
io.on('connection', (socket) => {
  ...
  socket.on('joinLobby', (data) => {
    const { playerId, playerRole, playerName, lobbyCode, drawTime } = data;
    ...
    if (lobbies[lobbyCode]) {
      if (lobbies[lobbyCode].players.length < 8) {
        ...
        lobbies[lobbyCode].drawTime = drawTime; // Store the draw time in the lobby
  ...
  socket.on('startGame', (lobbyCode) => {
    if (lobbies[lobbyCode]) {
      ...
      io.to(lobbyCode).emit('navigateToGame', gameCode, drawTime);
    }
  });
